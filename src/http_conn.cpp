...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    if (!(m_file_stat.st_mode & S_IROTH)) // S_IROTH表示是否其他用户拥有可读权限
        return FORBIDDEN_REQUEST;

    if (S_ISDIR(m_file_stat.st_mode)) //是否是目录而非文件
        return BAD_REQUEST;

    int fd = open(m_real_file, O_RDONLY); //打开文件，获取文件描述符
    //使用mmap映射到内存中
